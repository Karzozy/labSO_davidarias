...
[[maybe_unused]] void insertar_por_restante(std::deque<Proceso *> &cola, Proceso *p) {
    // Se busca la primera posicion cuyo restante ya sea mayor que el de p.
    // Insertar ahi deja a p despues de todos los que tienen restante igual
    // o menor, de modo que los empates los gana quien ya estaba en la cola.
...
Resultado planificar(std::vector<Proceso> &procesos, std::vector<Cola_prioridad> &colas, bool traza) {
    ...
    while (por_terminar > 0) {
        ...
        // Expropiacion de SRT: si durante este intervalo llega un proceso cuya
        // rafaga es menor que lo que le va quedando a "actual" en ese instante,
        // "actual" solo recibe CPU hasta que llega ese proceso.
        if (cola.estrategia == Estrategia::SRT) {
            for (Proceso *entrante : cola.llegada) {
                if (entrante->llegada >= ahora + quantum_asignado) break;
                int restante_de_actual_en_ese_instante = actual->restante - (entrante->llegada - ahora);
                if (entrante->restante < restante_de_actual_en_ese_instante) {
                    quantum_asignado = entrante->llegada - ahora;
        ...
        if (actual->restante == 0) {
            ...
            switch (cola.estrategia) {
                case Estrategia::FIFO:
                    ...
                    break;
                case Estrategia::RR:
                    // Agoto su quantum sin terminar: va al final de la cola.
                    cola.listos.push_back(actual);
                    break;
                case Estrategia::SJF:
                    // Sin expropiacion: conserva la CPU, igual que FIFO.
                    cola.listos.push_front(actual);
                    break;
                case Estrategia::SRT:
                    // Vuelve ordenado por su tiempo restante.
